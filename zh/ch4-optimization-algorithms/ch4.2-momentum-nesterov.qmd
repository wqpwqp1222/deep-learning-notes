...
```{python}
def run_momentum(theta_init: Tensor, lr: float, beta: float, steps: int) -> list[Tensor]:
    theta = theta_init.clone().requires_grad_()
    optimizer = SGDWithMomentum([theta], lr=lr, beta=beta)
    history = [theta.clone().detach()]
...
momentum_history = run_momentum(theta, lr=0.0035, beta=0.9, steps=40)
