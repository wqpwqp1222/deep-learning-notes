...
::: {.callout-note}
我们这里的实现和 PyTorch 标准的 `nn.MultiheadAttention` 有一点不同。`nn.MultiheadAttention` 把 `attn_dim` 同时作为 query 的输入维度、query/key/value 投影后的维度，以及 attention 的最终输出维度。它只额外提供 `kdim` 和 `vdim`，用来允许 key/value 的原始输入维度不同。
:::
