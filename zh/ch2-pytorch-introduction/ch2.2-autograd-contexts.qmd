...
## 2.2.3 `inference_mode()`：干脆以后都别记账了
...
这一节，我们讨论了 PyTorch 中的梯度记录控制。默认情况下，只要张量参与了可求导计算，Autograd 就会动态构建计算图。但在验证、推理或只需要数值结果的场景下，继续记录计算图往往是不必要的开销。

`torch.no_grad()` 可以临时关闭梯度记录，但不会改变张量本身的 `requires_grad` 属性；`torch.enable_grad()` 可以在局部重新开启梯度记录；`torch.set_grad_enabled()` 则可以根据条件动态控制是否记录梯度，常用于统一训练和评估代码。

相比之下，`torch.inference_mode()` 更适合纯推理场景。它不仅关闭计算图记录，还会跳过一些 Autograd 内部检查，因此通常更快、更省内存。但它的限制也更强：在 `inference_mode()` 中创建的张量，之后不能再重新加入自动微分系统。
